...
def dodaj_usun_klienta(wybor_klient, jak_usun, ID_do_usuniecia, NAME, EMAIL, PHONE):
    ...
    :param wybor_klient: Wybor miedzy "1" aby dodac klienta lub "2" aby go usunac.
    :param jak_usun: Wartosc wybor_klient musi byc rowna "2". Sposob usuniecia klienta. Wybor miedzy string "ID" lub "NAME"
    :param ID_do_usuniecia: ID klienta wykorzystywane do znalezienia i usnuniecia klienta z bazy danych.
    :param NAME: Imie klienta. Stosowane przy tworzeniu oraz usuwaniu.
...
def dodaj_address_klient(ID, STREET, CITY, COUNTRY):
    ...
    :param ID: ID klienta.
    :param STREET: Ulica na ktorej mieszka klient.
    :param CITY: Miasto w ktorym znajudje sie klient.
    :param COUNTRY: Narodowosc klienta.
    """
...
def wypozyczanie_ksiazek(id_klienta, id_ksiazki_input):
    ...
    :param id_klienta: ID klienta ktory wypozycza ksiazke.
    :param id_ksiazki_input: ID ksiazki ktora zostanie wypozyczona (wpisana do pliku tekstowego klienta).
    """
            ...
            for row in czytaj2:
                if int(row[0]) == int(id_ksiazki):  # Sprawdzamy czy ID ksiazki istnieje
                    ...
                    with open(plik, 'a') as plik_txt:  # Otwieramy plik
                        plik_txt.write(id_ksiazki + ", " + str(date.today()) + '\n')  # Dodajemy ID ksiazki do pliku tekstowego
...
def zwrot_ksiazki(id_klienta, id_ksiazki_input):
    ...
    :param id_klienta: ID klienta ktory zwraca ksiazke.
    :param id_ksiazki_input: ID ksiazki ktora bedzie zwrocona (usunieta z pliku tekstowego klienta).
    """
    with open('customer.csv', mode='r') as plik:
        czytaj = csv.reader(plik)
        wiersze = list(czytaj)
        ...
        with open('book.csv', mode='r') as plik_csv:
            ...
            if os.path.exists(plik):            # Jezeli klient byl zarejestrowany i plik tekstowy istnieje
                ...
                with open(plik, 'w') as plik_txt:
                    for line in lines:
                        if not line.startswith(id_ksiazki + ','):
                            plik_txt.write(line)
