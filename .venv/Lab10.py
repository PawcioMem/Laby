...
#     error_okno = Toplevel(window)
#     error_okno.title("BLAD")
#     Label(error_okno, text=message).pack()
#     Button(error_okno, text="OK", command=error_okno.destroy).pack()
#
# def callback():
#       try:
#           if var.get() == 1:
...
#
#
#           result_okno = Toplevel(window)
#           result_okno.title("Wynik")
#           Label(result_okno, text="Wynik: " + str(result)).pack()
#           Button(result_okno, text="OK", command=result_okno.destroy).pack()
#
#
#       except ValueError:
#           show_error("Blad wartosci! Upewnij sie ze uzywasz cyferek")
#
#       except ZeroDivisionError:
#           show_error("Nie mozna dzelic przez zero!")
#
#       except TypeError:
#           show_error('Nie wybrano operacji! Wybierz opcje po prawej')
#
#
# ##########################################
#
...
#
# Label(window, text="Pierwsza liczba =  ").grid(row=1, column=1)
# Label(window, text="Druga liczba =  ").grid(row=2, column=1)
# entry1 = Entry(window)
# entry1.grid(row=1, column=2)
# entry2 = Entry(window)
# entry2.grid(row=2, column=2)
...
# Radiobutton(window, text=" + ", variable=var, value=1).grid(row=1, column=3)
# Radiobutton(window, text=" - ", variable=var, value=2).grid(row=2, column=3)
# Radiobutton(window, text=" * ", variable=var, value=3).grid(row=3, column=3)
# Radiobutton(window, text=" / ", variable=var, value=4).grid(row=4, column=3)
#
# Button(window, text="Enter", command=callback).grid(row=4, column=1)
# window.mainloop()
...
Label(window, text="ANKIETA", bg="gold", fg="black").pack(side="top")
Label(window, text="Witamy w ankiecie dotyczącej kupowanych produktow na swieta").pack()
Label(window, text="Odpowiedzi sa anonimowe, prosimy wybrac pasujace dla panstwa opcje").pack()

Label(window, text="").pack()
Label(window, text="I. Co najczesciej kupujesz dla rodziny w prezencie?").pack()
